...
const Table = ({
    ...
    const handleDeleteWorker = (id, email) => {
        axiosInstance.delete(`/workers?id=${id}&&email=${email}`).then((res) => {
            res.data.deletedCount > 0 && setWorkerUpdated(true);
        });
    };

    // handle delete services
    const handleDeleteService = (id) => {
        axiosInstance.delete(`/services?id=${id}`).then((res) => {
            res.data.deletedCount > 0 && setIsDeleted(true);
        });
    };

    // handle approve application
    const handleApproveApplication = (worker) => {
        axiosInstance.put('/application', worker).then((res) => {
            setApplicationUpdate(true);
        });
    };
    // handle delete application
    const handleDeleteApplication = (email) => {
        axiosInstance.delete(`/application?email=${email}`).then((res) => {
            res.data.deletedCount && setApplicationUpdate(true);
        });
    };

    // handle working status
    const handleWorkingStatus = (email, status, id) => {
        axiosInstance.put('/workingStatus', { email, status, id }).then((res) => {
            res.data.message && setWorkUpdate(true);
        });
    };

    // handle complete work
    const handleCompleteWork = (id) => {
        axiosInstance.put('/complete', { id }).then((res) => {
    ...
    const handleDeleteToLet = (id) => {
        axiosInstance.delete(`/toLet/delete?id=${id}`).then((res) => {
            res.data.deletedCount && setToLetUpdated(true);
        });
    };
